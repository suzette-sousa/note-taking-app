    ...
    for(var i in data) {
      notesCtr.innerHTML +=  
        `<div class="notes-item-ctr">
          ...
          <form class="form notes-form-edit" onsubmit="editPost(event)" hidden>
            ...
            <button type="submit" class="btn-submit btn-submit-default">Modifier</button>
          </form>
        </div>
      `;
    }
  } else {
    document.getElementById("notes").append("Aucune note trouvée.");
  }
}
...
searchButton.addEventListener('submit', searchNote);

function searchNote(event) {
  event.preventDefault();
  const noteSearch = event.target.noteSearch.value;
  fetch(`/notes/search/${noteSearch}`)
    .then((response) => response.json())
    .then(function(data) {
      if(noteSearch) {
        displayNote(data);
    ...
    .then(function(err) {
        if(err.message) {
          var errorMsg = document.getElementById("error");
          errorMsg.innerHTML = "Erreur dans la saisie de votre recherche";
        } else {
          location.reload();
        }
      })
